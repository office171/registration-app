(function () {
  ...
  function createPaymentPanel(field) {
    ...
    const title = paid ? "התשלום התקבל" : verifying ? "בודק את אישור התשלום..." : "תשלום דמי הרשמה";
    ...
    panel.innerHTML = `
      <div class="payment-panel-content">
        ...
        ${failed ? `<span class="payment-warning">לא הצלחנו לאמת את התשלום. אם חויבתם, נסו לרענן או פנו למשרד.</span>` : ""}
    ...
    if (!paid && !verifying) {
      ...
      button.addEventListener("click", () => startRegistrationPayment(field));
  ...
  async function startRegistrationPayment(field) {
  ...
  async function handlePaymentReturn() {
    ...
    if (payment === "registration_success" && sessionId) {
      ...
      try {
        ...
        state.values.registration_payment_paid_at = verified.paidAt || new Date().toISOString();
        state.values.registration_payment_amount_usd = verified.amountUsd || 10;
      } catch {
  ...
  function validateStep() {
    ...
    step.fields.forEach((field) => {
      ...
      if (field.type === "payment" && field.required && !isRegistrationPaymentPaid()) {
  ...
  function reviewCards() {
    return [
      ...
      reviewCard("דמי הרשמה", "registration_payment", paymentReviewRows()),
  ...
  function paymentReviewRows() {
    return [
      ["סטטוס תשלום", isRegistrationPaymentPaid() ? "שולם" : "לא שולם"],
